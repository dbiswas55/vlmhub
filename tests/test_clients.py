...
def run(sample_dir: Path, clients: list[str], temperature: float | None = None) -> None:
    ...
    for name in clients:
        ...
        try:
            response = Model(name).generate(content, **overrides)
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    run(args.sample, args.clients, args.temperature)
